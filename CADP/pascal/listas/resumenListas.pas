{
    ...
    // RECORRER LISTA
    procedure recorrerLista(pI:lista); // pI es el "puntero inicial", que apunta a la cabeza de la lista 
    begin
    ...
    procedure agregarAdelante(var pI:lista; r:rDato); // se recibe la lista por referencia ya que hay que modificarla agregándole nodos, y el registro por valor porque solo hay que leerlo
                                // pI es el "puntero inicial", que apunta a la cabeza de la lista
    var
    ...
    begin
        ...
        if(pI = nil) then // si la lista está vacía...
            ...
            pI:=nuevo; // pero como estamos agregando adelante, ahora L va a apuntar al nodo nuevo
        end;
    end;
    ...
    begin
        ...
        if(pI = nil) then begin // si la lista está vacía...
            ...
            // es obvio, si la lista tiene un solo nodo, este es el primero y el último a la vez
        end
        else begin // si la lista no está vacía...
            pF^.sig:=nuevo; // el nodo que acabamos de crear ahora es el siguiente del último nodo de la lista
    ...
    // se utiliza una funcion ya que simplemente se debe retornar si se encontró o no el elemento buscado
    ...
    begin
        ...
        while((aux<>nil) and (encontre=false)) do begin // mientras que la lista no esté vacía y no haya encontrado nada...
            if(aux^.elem = valor) then // si el dato del nodo donde estamos parados es igual al valor buscado...
                encontre:=true; // encontre es verdadero
            else
                aux:=aux^.sig; // sino, aux apunta al siguiente nodo y sigue buscando a partir de ese
        end;
        buscarD:=encontre;
    end;
    // BUSCAR EN LISTA ORDENADA
    { Al buscar en una lista ordenada, hay cosas que ya sabemos. Como la lista está ordenada,
        no hace falta recorrerla toda para encontrar el elemento buscado. Solo se debe tener
        en cuenta hasta dónde debemos buscar, así cuando se llegue a ese elemento, no se busque más.
        Por ejemplo, si se busca el elemento "20" en una lista con elementos ordenados de menor a mayor:
                                [4] -> [12] -> [25] -> [32]
    ...
    begin
        encontre:=false;
        aux:=pI;
        while((aux <> nil) and (aux^.dato < valor)) do // mientras la lista no esté vacía y el dato del nodo actual sea menor a valor...
            aux:=aux^.sig; // ...entonces paso al siguiente nodo
        // sale del while o cuando se acabó la lista o cuando el dato del nodo ya no es menor a valor
        if((aux <> nil) and (aux^.dato = valor)) then // pero si no se acabó la lista y el dato del nodo es igual a valor...
